...
```{r EDAclinical, warning=FALSE, message=FALSE, out.width="700px",fig.align = "center" }

# Preliminary fast hand plots
# ------------------------------------------------------------------------------
knitr::include_graphics(path = "../results/03_EDA_grid4_combo2.png")
```

Figure: Exploratory analysis of clinical data: Mapping PAM50 tumor subtype category against other clinical data.
...
```{r boxplots, warning=FALSE, message=FALSE, out.width="600px",fig.align = "center"}
...
This was not feasible with such a large number of samples 
that display a large amount of differences even within each groups

Shown here are the common genes from top 50 Differentially Expressed  genes within each group,
calculated as an average value across all patients within that group. 

Still working on trying to see if this could be informative...

```{r, out.width="350px",,, out.width=c("49%","49%")}
knitr::include_graphics(c("../doc/Rmd_sup/df_full_top50_BioProc.png","../doc/Rmd_sup/df_full_top50_CellComponents.png"))

```


## Modelling data: PCA
```{r image_grobs, fig.align = "default", out.width=c("70%","40%"), fig.align="center" }
...
```{r}
knitr::include_graphics("../doc/Rmd_sup/Capture.PNG")
```
